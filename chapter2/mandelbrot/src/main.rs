...
fn main() {
    ...
    if args.len() != 5 {
        ...
        eprintln!(
            "Examples: {} mandel.png 1000x750 -1.20,0.35 -1,0.20",
            args[0]
        );
        std::process::exit(1);
    }

    let bounds = parse_pair(&args[2], 'x').expect("failed to read the bounds");
    let upper_left = parse_complex(&args[3]).expect("failed to get the upper left corner point");
    let lower_right = parse_complex(&args[4]).expect("failed to get the lower right corner point");
